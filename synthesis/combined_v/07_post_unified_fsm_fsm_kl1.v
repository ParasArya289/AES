...
module aes_kexp (
	...
	generate
		for (_gv_i_4 = 1; _gv_i_4 < Nx; _gv_i_4 = _gv_i_4 + 1) begin : genblk2
			...
			assign sw_in_i = RotWord(KExp_P[((9 - i) * 6) * 32+:32]);
			wire [31:0] sw_in_i2;
			wire [31:0] sw_out_i2;
			aes_cfa_sbox sw_i4(
				.a_in(sw_in_i2[31:24]),
			...
			for (_gv_j_1 = 0; _gv_j_1 < aes_const_Nk; _gv_j_1 = _gv_j_1 + 1) begin : genblk1
				...
				if ((j % aes_const_Nk) == 0) begin : genblk1
					wire [32:1] sv2v_tmp_E426E;
					assign sv2v_tmp_E426E = (KExp_N[(((9 - i) * 6) + (5 - j)) * 32+:32] ^ sw_out_i) ^ {rcon[(15 - i) * 8+:8], 24'h000000};
					always @(*) KExp_P[(((8 - i) * 6) + (5 - j)) * 32+:32] = sv2v_tmp_E426E;
				end
				else begin : genblk1
		...
		case (r[11-:4])
			...
			default: begin
				begin : sv2v_autoblock_2
					...
					for (i = 0; i < aes_const_Nk; i = i + 1)
						if (i == 0)
							KExp_P[(5 - i) * 32+:32] = (KExp_N[(5 - i) * 32+:32] ^ sw_out_rot) ^ {rcon[(15 - v[11-:4]) * 8+:8], 24'h000000};
						else
