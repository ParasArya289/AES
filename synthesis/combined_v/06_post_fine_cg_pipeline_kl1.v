module aes_xkey (
	...
	localparam aes_const_Nk = 6;
	input wire [191:0] key_in;
	output wire [191:0] key_out;
	genvar _gv_i_1;
	generate
		for (_gv_i_1 = 5; _gv_i_1 >= 0; _gv_i_1 = _gv_i_1 - 1) begin : genblk1
			localparam i = _gv_i_1;
			assign key_out[(23 - (4 * ((aes_const_Nk - i) - 1))) * 8+:8] = key_in[(32 * (i + 1)) - 1:(32 * i) + 24];
			assign key_out[(23 - ((4 * ((aes_const_Nk - i) - 1)) + 1)) * 8+:8] = key_in[(32 * (i + 1)) - 9:(32 * i) + 16];
			assign key_out[(23 - ((4 * ((aes_const_Nk - i) - 1)) + 2)) * 8+:8] = key_in[(32 * (i + 1)) - 17:(32 * i) + 8];
			assign key_out[(23 - ((4 * ((aes_const_Nk - i) - 1)) + 3)) * 8+:8] = key_in[(32 * (i + 1)) - 25:32 * i];
		end
	endgenerate
endmodule
module aes_xdata (
...
module aes_kexp (
	...
	localparam aes_const_Nb = 4;
	localparam aes_const_Nr = 12;
	output reg [1663:0] KExp;
	...
	function automatic [191:0] sv2v_cast_B6EF8;
		input reg [191:0] inp;
		sv2v_cast_B6EF8 = inp;
	endfunction
	initial begin
		...
		for (_gv_i_4 = 0; _gv_i_4 < aes_const_Nk; _gv_i_4 = _gv_i_4 + 1) begin : genblk1
			...
			always @(*) begin
				if (_sv2v_0)
					;
				if (Enable == 1)
					KExp_P[(48 + (5 - i)) * 32+:32] = {Key[(23 - (4 * i)) * 8+:8], Key[(23 - ((4 * i) + 1)) * 8+:8], Key[(23 - ((4 * i) + 2)) * 8+:8], Key[(23 - ((4 * i) + 3)) * 8+:8]};
			end
		end
	endgenerate
	always @(*) begin
		if (_sv2v_0)
			;
		KExp_R[1472+:192] = KExp_P[1536+:192];
		Ready_P[0] = Enable;
	end
	...
	always @(posedge clk)
		if (rst == 0) begin
			KExp_N[1536+:192] <= {aes_const_Nk {sv2v_cast_32({32 {1'b0}})}};
			Ready_N[0] <= 1'sb0;
		end
		else if (Enable || Ready_N[0]) begin
			KExp_N[1536+:192] <= KExp_P[1536+:192];
			Ready_N[0] <= Ready_P[0];
		end
	generate
		for (_gv_i_4 = 1; _gv_i_4 < Nx; _gv_i_4 = _gv_i_4 + 1) begin : genblk2
			...
			for (_gv_j_1 = 0; _gv_j_1 < aes_const_Nk; _gv_j_1 = _gv_j_1 + 1) begin : genblk1
				...
				if ((j % aes_const_Nk) == 0) begin : genblk1
					wire [32:1] sv2v_tmp_105A1;
					assign sv2v_tmp_105A1 = (KExp_N[(((9 - i) * 6) + (5 - j)) * 32+:32] ^ SubWord(RotWord(KExp_P[((9 - i) * 6) * 32+:32]))) ^ {RCon[(15 - i) * 8+:8], 24'h000000};
					always @(*) KExp_P[(((8 - i) * 6) + (5 - j)) * 32+:32] = sv2v_tmp_105A1;
				end
				else begin : genblk1
					wire [32:1] sv2v_tmp_3703C;
					assign sv2v_tmp_3703C = KExp_N[(((9 - i) * 6) + (5 - j)) * 32+:32] ^ KExp_P[(((8 - i) * 6) + (6 - j)) * 32+:32];
					always @(*) KExp_P[(((8 - i) * 6) + (5 - j)) * 32+:32] = sv2v_tmp_3703C;
				end
			end
			always @(*) begin
				if (_sv2v_0)
					;
				KExp_R[32 * (51 - ((aes_const_Nk * i) >= (Min(aes_const_Nk * (i + 1), 52) - 1) ? aes_const_Nk * i : ((aes_const_Nk * i) + ((aes_const_Nk * i) >= (Min(aes_const_Nk * (i + 1), 52) - 1) ? ((aes_const_Nk * i) - (Min(aes_const_Nk * (i + 1), 52) - 1)) + 1 : ((Min(aes_const_Nk * (i + 1), 52) - 1) - (aes_const_Nk * i)) + 1)) - 1))+:32 * ((aes_const_Nk * i) >= (Min(aes_const_Nk * (i + 1), 52) - 1) ? ((aes_const_Nk * i) - (Min(aes_const_Nk * (i + 1), 52) - 1)) + 1 : ((Min(aes_const_Nk * (i + 1), 52) - 1) - (aes_const_Nk * i)) + 1)] = KExp_P[32 * ((((((8 - i) * 6) + (6 - (Min(aes_const_Nk * (i + 1), 52) - (aes_const_Nk * i)))) + (Min(aes_const_Nk * (i + 1), 52) - (aes_const_Nk * i))) - 1) - ((Min(aes_const_Nk * (i + 1), 52) - (aes_const_Nk * i)) - 1))+:32 * (Min(aes_const_Nk * (i + 1), 52) - (aes_const_Nk * i))];
				Ready_P[i] = Ready_N[i - 1];
			end
			always @(posedge clk)
				if (rst == 0) begin
					KExp_N[32 * ((8 - i) * 6)+:192] <= {aes_const_Nk {sv2v_cast_32({32 {1'b0}})}};
					Ready_N[i] <= 1'sb0;
				end
				else if (Ready_N[i - 1]) begin
					KExp_N[32 * ((8 - i) * 6)+:192] <= KExp_P[32 * ((8 - i) * 6)+:192];
					Ready_N[i] <= Ready_P[i];
				end
		end
	endgenerate
...
module aes_kexp_state (
	...
	always @(r[0] or KExp_R or v or v[7-:6] or KExp_P or v[7-:6] or v[1] or v[7-:6] or v[11-:4] or v[7-:6] or KExp_P or KExp_N or KExp_P or SBox or SBox or SBox or SBox or KExp_N or v[11-:4] or RCon or KExp_N[0+:32] or SBox or SBox or SBox or SBox or KExp_N or Key or Key or Key or Key or Enable or r[11-:4] or r or _sv2v_0) begin
				...
				if (Enable == 1) begin
					...
					begin : sv2v_autoblock_1
						...
						for (i = 0; i < aes_const_Nk; i = i + 1)
							KExp_P[(5 - i) * 32+:32] = {Key[(23 - (4 * i)) * 8+:8], Key[(23 - ((4 * i) + 1)) * 8+:8], Key[(23 - ((4 * i) + 2)) * 8+:8], Key[(23 - ((4 * i) + 3)) * 8+:8]};
					end
				end
			end
			default: begin
				begin : sv2v_autoblock_2
					...
					for (i = 0; i < aes_const_Nk; i = i + 1)
						if (i == 0)
							KExp_P[(5 - i) * 32+:32] = (KExp_N[(5 - i) * 32+:32] ^ SubWord(RotWord(KExp_N[0+:32]))) ^ {RCon[(15 - v[11-:4]) * 8+:8], 24'h000000};
						else
							KExp_P[(5 - i) * 32+:32] = KExp_N[(5 - i) * 32+:32] ^ KExp_P[(6 - i) * 32+:32];
				end
				if (v[7-:6] > 46) begin
					v[11-:4] = 0;
					v[0] = 1;
		...
		begin : sv2v_autoblock_3
			reg signed [31:0] i;
			for (i = 0; i < aes_const_Nk; i = i + 1)
				if (((v[7-:6] + i[5:0]) < 52) && (v[1] == 1))
					KExp_R[(51 - (v[7-:6] + i[5:0])) * 32+:32] = KExp_P[(5 - i) * 32+:32];
		end
	...
	generate
		for (_gv_j_2 = 0; _gv_j_2 < aes_const_Nb; _gv_j_2 = _gv_j_2 + 1) begin : genblk1
			...
			for (_gv_i_5 = 0; _gv_i_5 < 4; _gv_i_5 = _gv_i_5 + 1) begin : genblk1
				...
				assign State_out[(15 - ((4 * j) + i)) * 8+:8] = KExp[((51 - ((Index * aes_const_Nb) + j)) * 32) + ((((4 - i) * 8) - 1) >= ((3 - i) * 8) ? ((4 - i) * 8) - 1 : ((((4 - i) * 8) - 1) + ((((4 - i) * 8) - 1) >= ((3 - i) * 8) ? ((((4 - i) * 8) - 1) - ((3 - i) * 8)) + 1 : (((3 - i) * 8) - (((4 - i) * 8) - 1)) + 1)) - 1)-:((((4 - i) * 8) - 1) >= ((3 - i) * 8) ? ((((4 - i) * 8) - 1) - ((3 - i) * 8)) + 1 : (((3 - i) * 8) - (((4 - i) * 8) - 1)) + 1)] ^ State_in[(15 - ((4 * j) + i)) * 8+:8];
			end
		end
	endgenerate
endmodule
...
module aes_cipher (
	...
	reg [0:0] Ready [0:11];
	reg [0:0] Ready_Reg [0:11];
	wire [127:0] sbyte_in_muxed [0:11];
	aes_arkey aes_arkey_comp(
		.State_in(Data_in),
		.KExp(KExp),
		.Index(4'h0),
	...
	always @(posedge clk)
		if (rst == 0)
			Ready_Reg[11] <= 1'sb0;
		else if (Ready_Reg[10] || Ready_Reg[11]) begin
			State_Reg[11] <= State[11];
			Ready_Reg[11] <= Ready[11];
		end
	assign sbyte_in_muxed[11] = (Ready_Reg[11] ? State_Reg[11] : {16 {8'b00000000}});
	aes_fround aes_fround_comp(
		.State_in(sbyte_in_muxed[11]),
...
module aes_icipher (
	...
	reg [0:0] Ready [0:11];
	reg [0:0] Ready_Reg [0:11];
	wire [127:0] isbyte_in_muxed [0:11];
	aes_arkey aes_arkey_comp(
		.State_in(Data_in),
		.KExp(KExp),
		.Index(aes_const_Nr[3:0]),
	...
	always @(posedge clk)
		if (rst == 0)
			Ready_Reg[11] <= 1'sb0;
		else if (Enable || Ready_Reg[11]) begin
			State_Reg[11] <= State[11];
			Ready_Reg[11] <= Ready[11];
		end
	assign isbyte_in_muxed[11] = (Ready_Reg[11] ? State_Reg[11] : {16 {8'b00000000}});
	aes_iround aes_iround_top_comp(
		.State_in(isbyte_in_muxed[11]),
		.Index(4'sd11),
		.KExp(KExp),
		.State_out(State[10])
	);
	always @(*) begin
		if (_sv2v_0)
			;
		Ready[10] = Ready_Reg[11];
	end
	generate
		for (_gv_i_11 = 10; _gv_i_11 > 0; _gv_i_11 = _gv_i_11 - 1) begin : genblk1
...
module aes (
	...
	reg [0:0] kexp_pending;
	reg [0:0] cipher_pending;
	reg [0:0] icipher_pending;
	...
	always @(posedge clk)
		if (rst == 0) begin
			kexp_pending <= 1'sb0;
			cipher_pending <= 1'sb0;
			icipher_pending <= 1'sb0;
		end
		else begin
			if (kexp_enable)
				kexp_pending <= 1;
	...
	always @(*) begin
		...
		else if ((cipher_ready == 1) && (cipher_pending == 1)) begin
			aes_out[128-:128] = cipher_data;
			aes_out[0] = 1;
		end
		else if ((icipher_ready == 1) && (icipher_pending == 1)) begin
