module aes_arkey (
	...
	genvar _gv_j_1;
	generate
		for (_gv_j_1 = 0; _gv_j_1 < aes_const_Nb; _gv_j_1 = _gv_j_1 + 1) begin : genblk1
...
module aes_cipher (
	...
	generate
		for (_gv_i_3 = 2; _gv_i_3 < aes_const_Nr; _gv_i_3 = _gv_i_3 + 1) begin : genblk1
	...
	always @(posedge clk)
		if (rst == 0)
			Ready_Reg[9] <= 1'sb0;
		else if (Ready_Reg[8] || Ready_Reg[9]) begin
			State_Reg[9] <= State[9];
			Ready_Reg[9] <= Ready[9];
		end
	assign sbyte_in_muxed[9] = (Ready_Reg[9] ? State_Reg[9] : {16 {8'b00000000}});
	aes_fround aes_fround_comp(
		.State_in(sbyte_in_muxed[9]),
...
module aes_icipher (
	...
	reg [0:0] Ready [0:9];
	reg [0:0] Ready_Reg [0:9];
	wire [127:0] isbyte_in_muxed [0:9];
	aes_arkey aes_arkey_comp(
		.State_in(Data_in),
		.KExp(KExp),
		.Index(aes_const_Nr[3:0]),
	...
	always @(posedge clk)
		if (rst == 0)
			Ready_Reg[9] <= 1'sb0;
		else if (Enable || Ready_Reg[9]) begin
			State_Reg[9] <= State[9];
			Ready_Reg[9] <= Ready[9];
		end
	assign isbyte_in_muxed[9] = (Ready_Reg[9] ? State_Reg[9] : {16 {8'b00000000}});
	aes_iround aes_iround_top_comp(
		.State_in(isbyte_in_muxed[9]),
		.Index(4'sd9),
		.KExp(KExp),
		.State_out(State[8])
	);
	...
	generate
		for (_gv_i_4 = 8; _gv_i_4 > 0; _gv_i_4 = _gv_i_4 - 1) begin : genblk1
...
module aes_isbyte (
	...
	genvar _gv_i_5;
	generate
		for (_gv_i_5 = 0; _gv_i_5 < 16; _gv_i_5 = _gv_i_5 + 1) begin : gen_cfa_isbox
...
module aes_isrow (
	...
	generate
		...
		for (_gv_j_2 = 0; _gv_j_2 < aes_const_Nb; _gv_j_2 = _gv_j_2 + 1) begin : genblk2
			localparam j = _gv_j_2;
			for (_gv_i_6 = 1; _gv_i_6 < 4; _gv_i_6 = _gv_i_6 + 1) begin : genblk1
		...
		case (r[11-:4])
			...
			default: begin
				begin : sv2v_autoblock_2
					...
					for (i = 0; i < aes_const_Nk; i = i + 1)
						if (i == 0)
							KExp_P[(3 - i) * 32+:32] = (KExp_N[(3 - i) * 32+:32] ^ sw_out_rot) ^ {rcon[(15 - v[11-:4]) * 8+:8], 24'h000000};
						else
...
module aes_kexp (
	...
	genvar _gv_j_3;
	generate
		for (_gv_i_7 = 0; _gv_i_7 < aes_const_Nk; _gv_i_7 = _gv_i_7 + 1) begin : genblk1
	...
	always @(posedge clk)
		if (rst == 0) begin
			KExp_N[1280+:128] <= {aes_const_Nk {sv2v_cast_32({32 {1'b0}})}};
			Ready_N[0] <= 1'sb0;
		end
	...
	generate
		for (_gv_i_7 = 1; _gv_i_7 < Nx; _gv_i_7 = _gv_i_7 + 1) begin : genblk2
			localparam i = _gv_i_7;
			wire [31:0] sw_in_i;
			wire [31:0] sw_out_i;
			aes_cfa_sbox sw_i0(
			...
			assign sw_in_i = RotWord(KExp_P[((11 - i) * 4) * 32+:32]);
			wire [31:0] sw_in_i2;
			wire [31:0] sw_out_i2;
			aes_cfa_sbox sw_i4(
				.a_in(sw_in_i2[31:24]),
			...
			for (_gv_j_3 = 0; _gv_j_3 < aes_const_Nk; _gv_j_3 = _gv_j_3 + 1) begin : genblk1
				localparam j = _gv_j_3;
				if ((j % aes_const_Nk) == 0) begin : genblk1
					wire [32:1] sv2v_tmp_8D2A4;
					assign sv2v_tmp_8D2A4 = (KExp_N[(((11 - i) * 4) + (3 - j)) * 32+:32] ^ sw_out_i) ^ {rcon[(15 - i) * 8+:8], 24'h000000};
					always @(*) KExp_P[(((10 - i) * 4) + (3 - j)) * 32+:32] = sv2v_tmp_8D2A4;
				end
				else begin : genblk1
			...
			always @(posedge clk)
				if (rst == 0) begin
					KExp_N[32 * ((10 - i) * 4)+:128] <= {aes_const_Nk {sv2v_cast_32({32 {1'b0}})}};
					Ready_N[i] <= 1'sb0;
				end
...
module aes_sbyte (
	...
	genvar _gv_i_8;
	generate
		for (_gv_i_8 = 0; _gv_i_8 < 16; _gv_i_8 = _gv_i_8 + 1) begin : gen_cfa_sbox
...
module aes_tb_vivado;
	...
	localparam [127:0] KEY = 128'hdffe1e8f4824951ba6d3bf8b46e53c4d;
	localparam [127:0] PLAIN = 128'hbab8d38cebb576d489d7606e10784ddc;
	localparam [127:0] CIPHER = 128'h09036baae31b28e2b6878d016358b083;
	integer state = 0;
	reg enable = 1;
	reg [127:0] result;
	integer tests_run = 0;
	...
	always @(posedge clk)
		if (!rst) begin
			...
			if (aes_out[0]) begin
				enable <= 1;
				state <= 2;
				result <= aes_out[128-:128];
				tests_run <= tests_run + 1;
				$display("[TB] ENCRYPT:  %032x", aes_out[128-:128]);
				$display("[TB] EXPECTED: %032x", CIPHER);
				if (aes_out[128-:128] == CIPHER) begin
			...
			if (aes_out[0]) begin
				...
				state <= 3;
				tests_run <= tests_run + 1;
				$display("[TB] DECRYPT:  %032x", aes_out[128-:128]);
				$display("[TB] EXPECTED: %032x", PLAIN);
				if (aes_out[128-:128] == PLAIN) begin
...
module aes_tb (
	...
	integer state;
	integer i;
	reg [127:0] key_block [0:0];
	localparam aes_const_Nw = 1;
	reg [127:0] data_block [0:0];
	reg [127:0] encrypt_block [0:0];
	initial begin
	...
	always @(posedge clk)
		if (rst == 0) begin
			...
			aes_in[258-:128] <= key_block[0];
			aes_in[130-:128] <= 0;
			aes_in[2-:2] <= 1;
			aes_in[0] <= enable;
			enable <= 0;
			if (aes_out[0] == 1) begin
				enable <= 1;
				state <= state + 1;
				$write("%c[1;34m", 8'h1b);
				$write("KEY: ");
			...
			if (aes_out[0] == 1) begin
				...
				$write("ENCRYPT: ");
				$write("%c[0m", 8'h1b);
				$display("%x", aes_out[128-:128]);
				$write("%c[1;34m", 8'h1b);
				$write("CORRECT: ");
				$write("%c[0m", 8'h1b);
				$display("%x", encrypt_block[i]);
				if (|(aes_out[128-:128] ^ encrypt_block[i]) == 0) begin
					$write("%c[1;32m", 8'h1b);
			...
			if (aes_out[0] == 1) begin
				...
				orig <= aes_out[128-:128];
				if (|(aes_out[128-:128] ^ data_block[i]) == 0) begin
					$write("%c[1;32m", 8'h1b);
					$display("TEST SUCCEEDED");
					$write("%c[0m", 8'h1b);
				end
	...
	generate
		if (enable_pipeline) begin : genblk1
			aes aes_comp(
				.rst(rst),
				.clk(clk),
				.aes_in(aes_in),
...
module aes_xdata (
	...
	genvar _gv_i_10;
	generate
		for (_gv_i_10 = 3; _gv_i_10 >= 0; _gv_i_10 = _gv_i_10 - 1) begin : genblk1
...
module aes_xkey (
	...
	genvar _gv_i_11;
	generate
		for (_gv_i_11 = 3; _gv_i_11 >= 0; _gv_i_11 = _gv_i_11 - 1) begin : genblk1
